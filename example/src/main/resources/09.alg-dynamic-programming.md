...
状态转移方程：
L(m,n) =  
1. L(m-1,n-1)+1, 当text1[m-1]==text2[n-1]
...
bobo老师说dijkstra单源最短路径算法也是动态规划！！！有空去了解下！`todo`
...
读清楚题目以后，这题就很简单啦：先将数组排序，然后相邻的两个元素交换位置就可以了。代码：
```java
public int[] numberGame(int[] nums) {
...
这题用中心扩散法来做，逐个遍历s中的字符，然后以当前字符为中心，向左右扩散，找到最长的回文子串。
需要注意的是，以当前字符为中心，回文串有可能是奇数长度，也有可能是偶数长度。
代码：
```java
public String longestPalindrome(String s) {
    String res = "";
...
```
最长回文子串问题也可以通过动态规划的方法高效解决。动态规划的关键在于构建一个二维数组dp，其中`dp[i][j]`表示字符串s从索引i到j的子串是否为回文串。
动态规划的状态转移方程如下：
当`s[i]==s[j]`并且`j - i <= 2 `或者 `dp[i + 1][j - 1]` 为真时，`dp[i][j]`为真。
否则，`dp[i][j]`为假。
下面是一个使用 Java 编写的动态规划解法的示例代码：
```java
public String longestPalindrome(String s) {
...
这题用动态规划来做，定义状态`dp_odd[i][j]`表示从cards[0...i]中抽取j+1张牌能获取到的总和为奇数的最大分数，`dp_even[i][j]`表示从cards[0...i]中抽取j+1张牌能获取到的总和为偶数的最大分数。
那么这题的答案就是`dp_even[cards.length-1][cnt-1]`。
对于`cards[i]`这张卡片，我们面临两种选择：
1. 我们可以选择它
2. 我们也可以选择不选它。
由此，状态转移方程为：
`dp_even[i][j]` = max(`dp_even[i-1][j]`, (pre = (`cards[i]`为偶数 ? `dp_even[i-1][j-1]` : `dp_odd[i-1][j-1]`)) == 0 ? 0 : (pre + `cards[i]`))
`dp_odd[i][j]` = max(`dp_odd[i-1][j]`, (pre = (`cards[i]`为奇数 ? `dp_even[i-1][j-1]` : `dp_odd[i-1][j-1]`)) == 0 ? 0 : (pre + `cards[i]`))
其中0表示无法组成奇/偶总和。base case 要根据实际情况遍历设置。
代码：
```java
public int maxmiumScore(int[] cards, int cnt) {
...
```
以上题解的复杂度是O(n *log n)。
以上解法还可以使用空间换取时间，将复杂度降低O(n)。思路：
和上面的方法思路一致，但时间复杂度的开销主要在排序上，还可以进一步优化。
注意到 cards[i] 的取值范围为 [1,1000]，因此可以开辟一个 val 数组，val[k] 表示值为 k 的元素个数。将遍历 cards 改为遍历 val，处理思路与方案一一致不再赘述。
代码：
```java
public int maxmiumScore(int[] cards, int cnt) {
...
```
这个用空间替代排序的解法，在数据范围比较小的时候，还是很有用呢~
自己按照思路写一遍：
```java
/**
 * LCP 40: 心算挑战
...
###### 问题3129：找出所有稳定的二进制数组I

###### 问题2144：分割字符频率相等的最少子字符串